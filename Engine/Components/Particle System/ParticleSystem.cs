...
namespace Engine
{
    public class ParticleSystem : Component
    {
        ...
        [ShowInEditor] public Vector2 StartVelocity { get; set; } = new Vector2(100, -200);
        ...
        void BuildShape()
        {
            for (int i = 0; i < MaxParticles; i++)
            {
                ...
                float val = ((float)i / MaxParticles);
                float sineX = (float)Math.Cos(val * Math.PI * 2) * radius;
                float sineY = (float)Math.Sin(val * Math.PI * 2) * radius;

                Vector2 wiggle = new Vector2(sineX, sineY).NormalizedCopy() * (float)Math.Sin(val) * 100 * 0;
                ...
                p.color = new Color((int)p.color.R, p.color.G, p.color.B, 150);
                particles.Add(p);
                //pool.PutObject(p);
            }
        }
        public void Upddfdate()
        {

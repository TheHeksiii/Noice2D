...
namespace Engine
{
	...
	public class GameObject
	{
		...
		public GameObject Parent
		{
			get
			{
				int index = Scene.Instance.GetGameObjectIndex(parentID);
				if (index != -1)
				{ return Scene.Instance.gameObjects[index]; }
				else
				{
					return null;
				}
			...
			set
			{
				...
				if (index != -1)
				{
					Scene.Instance.gameObjects[Scene.Instance.GetGameObjectIndex(parentID)] = value;
				}
			}
		}
		...
		void Setup(bool linkComponents = true)
		{
			...
			Scene.Instance.OnGameObjectCreated(this);
		}
		...
		private void RemoveFromLists(GameObject gameObject)
		{
			...
			Scene.Instance.OnGameObjectDestroyed(this);
		}

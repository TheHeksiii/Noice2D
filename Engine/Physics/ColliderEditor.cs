    ...
    public class ColliderEditor
    {
        ...
        public void ToggleEditing()
        {
            ...
            if (editing)
            {
                ...
                return;
                collider = null;//Editor.GetInstance().GetSelectedGameObject()?.GetComponent<Collider>();
            ...
            else// no point selected
            {
                ...
                if (overVertex == false)
                {
                    ...
                    if (Vector2.Distance(pointOnLine, MouseInput.Position) < 10) // dont do anything if we are too far away from line
                    {
                        ...
                        dynamicVertex.transform.Position = (pointOnLine.ToSize() - dynamicVertex.GetComponent<BoxCollider>().rect.Size / 2);
                    }
                    else
                    {
                        dynamicVertex.Active = false;
